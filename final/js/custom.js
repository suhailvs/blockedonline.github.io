$(window).load(function(){
  ...
  var $selectize2=$('#select-url').selectize({
    ...
    load: function (query, callback) {
      ...
      $.ajax({
        url: 'http://api.blockedonline.com/blocked/data?url_query=' + encodeURIComponent(query),
        type: 'GET',
...
function DoAjax_Country(country,opt,box){
  $.get("http://api.blockedonline.com/blocked/data",{country: country,v:opt})
  .done(function(datas) {    
    if (datas.status == 'OK'){
      if (box=='donutgraph'){
        //draw donut graph
...
function DoAjax_Url(urlname,opt){
  ...
  $.get("http://api.blockedonline.com/blocked/data",{url: urlname,v:opt})
  .done(function(datas) {    
    if (datas.status == 'OK'){   
